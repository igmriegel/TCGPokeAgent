...
def _(
    ...
    for _s in submissions:
        ...
        sub_rows += (
            f"<tr><td>{_s['label']} {_badge}</td><td>{_s['id']}</td>"
...
<div class="metric-row">
  ...
  <div class="metric">
    <div class="metric-label">Avg Turns (Win)</div>
    <div class="metric-value win">{avg_turns_win:.1f}</div>
  </div>
  <div class="metric">
    <div class="metric-label">Avg Turns (Loss)</div>
    <div class="metric-value loss">{avg_turns_loss:.1f}</div>
  </div>
</div>

<div class="insight">
  <strong>Key Insight:</strong> Loss games end
  <strong>{avg_turns_win - avg_turns_loss:.0f} turns faster</strong>
  ({avg_turns_loss:.1f} vs {avg_turns_win:.1f}). Win games deal
  <strong>{_avg(dmg_dealt_win) / max(_avg(dmg_taken_win), 1):.1f}x more damage</strong>
  than taken.
</div>
