...
## 2026-08-12 — Submission 55422182 first-divergence review

Decision ledgers downloaded and decoded for submission `55422182` identify one
first actionable divergence per replay. These are single-decision observations,
not claims about counterfactual wins. Replay and submission IDs are provenance
only and never participate in runtime policy.

| Replay | First observed divergence | Public evidence | Local correction scope |
| --- | --- | --- | --- |
| `92410349` | Passive END retained Roto-Stick and Articuno. | Decision 26 selected END while both cards were playable candidates. | A supporterless Roto remains productive unless the deck-reserve veto applies. |
| `92351382` | Ariana was selected before the hand had a complete auditable reduction pass. | Decision 2 selected Ariana while Giovanni and Energy candidates remained; the historical ledger had no retained-card reasons. | Stadium, development, committed Poké Pad, productive items, and productive Energy are considered before Ariana; retained cards have a ledger field. |
| `92344156` | Passive END retained the sole Roto-Stick. | Decision 10 selected END with Roto-Stick as the only non-END candidate. | A supporterless Roto attempts conversion rather than ending passively. |
| `92301028` | Ariana preceded a Roto-Stick held in a no-attack resource line. | Decision 20 selected Ariana while Roto-Stick and Articuno were playable candidates. | The canonical loop allows Roto before a Supporter when no Supporter is in hand and reserve permits it. |
| `92280407` | Petrel was selected despite two playable Arianas and Proton already in hand; the following turn repeated Petrel into Proton and passed. | Owner-reviewed replay sequence. | Petrel rejects Proton already held; its Factory exception applies only when Ariana draws at most two cards. |
| `92269436` | A post-KO Archer displaced a public Giovanni/Roto attack line; the later low-hand Headset opportunity was not treated as Ariana recovery. | Owner-reviewed replay sequence. | Archer is vetoed by public attack/setup/draw lines; Giovanni evaluates Bench attackers; Headset accepts a small inactive hand with Ariana in discard. |
...
The following raw replays were inspected at their first causal owner-side
actions. They establish observed sequencing inputs only and do not establish an
alternate result.

| Replay | Observed public sequence | Divergence class | Local guard |
| --- | --- | --- | --- |
| `92201785` | Ultra Ball search preceded Porygon2 selection during opening setup. | `setup_search_before_proton` | Proton/setup blocks Ultra Ball, and Ultra Ball cannot select Porygon2 in that state. |
...
The decision ledger exposes the corresponding guard or priority reason:
